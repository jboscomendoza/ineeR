...
#'
#' Estima porcentajes de variables con su error estandar e intervalos de
#' confianza al 95%.
#'
#' @param tabla Data frame u objeto coercionable a data.frame con datos de
#'   Excale o Planea.
#' @param variable Cadena de texto con la variable de la que se estimaran
#'   porcentajes.
#' @param prefijo Cadena de texto con el prefijo de las variables que contienen
#'   los pesos muestrales replicados.
#' @param peso_final Cadena de texto con el nombre de la variable que contiene
#'   el peso muestral combinado.
#' @param grupo Cadena de texto opcional con el nombre de la variable o
#'   variables de agrupacion que se usaran para estimar porcentajes. Por
#'   defecto, ninguna.
#' @param cifras_decimales Numero de cifras decimales que se mostraran en los
#'   resultados.
#'
#' @return Una Data frame que contiene la estimacion de porcentaje para el
#'   argumento variable, la estimacion del error estandar asociado a cada
#'   porcentaje y los limites inferior y superior de su intervalo de confianza
#'   al 95%.
#'
#' @examples
...
#'
#' # Es posible pedir porcentajes por mas de un grupo, pero esto produce estimaciones con errores altos.
#' #' estimacion_porcentaje(tabla = planea2015, variable = "SEXO",
...
estimacion_porcentaje <- function(tabla, variable, prefijo, peso_final, grupo = NULL, cifras_decimales = 2) {
    calculos <- function(elemento) {
      ...
      vec_pesos <- append(peso_final, grep(x = names(elemento), pattern = prefijo,
                                           value = T))
      vec_sumatoria <- as.vector(sapply(elemento[vec_pesos], function(peso) {
          sum(peso, na.rm = T)
        }))
